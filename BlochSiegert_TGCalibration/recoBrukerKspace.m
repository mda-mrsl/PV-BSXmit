function [img, ksp, faFids, faNoise, faKraw, faSats] = ...
...
%       [img, ksp] = recoBrukerKspace(b, acqp, method, showImgs)
%     Fidall Experiment:
%       [img, ksp, faFids, faNoise, faKraw, faSats] = ...
%           recoBrukerKspace(brukerFid, acqp, method, showImgs, idealHz)
%
...
%               default 1 (passed directly to SHAPEBRUKERFIDALL)
%             idealHz contains the frequency offsets to use in IDEAL decomp
%               default [] (PLHUAB)
%
%   See also READBRUKERHEADER, READBRUKERFID, SHAPEBRUKERFIDALL
%
...
switch ppg
    ...
    case {'spflash.ppg', 'singlepulse.ppg', 'powcalsinglepulse.ppg', ...
            'nspect.ppg', 'press.ppg', 'bssinglepulse.ppg', 'mrslnspect.ppg'}
        % Try initial reshape
        try
    ...
    case 'fidall.ppg'
        ...
        try
            ...
            error('recoBrukerKspace:fidall', ...
                'Failed to reshape %s MR data', ppg)
        end
        [ksp, faFids, faNoise, faKraw, faSats] = shapeBrukerFidall(...
...
if ~isempty(faFids) && showImgs == 1
    plotSpec(ifftdim(faFids, 1), acqp.SW_h, true);
    set(gcf, 'Name', sprintf('Fidall Slice Spectra - %s', acqp.ACQ_scan_name))
end
...
if isdeployed
    if showImgs == 1
        ...
        %     mfilename, datestr(now, 30))), 'img', 'ksp', 'fa*')
        if strcmp(ppg, 'fidall.ppg')
            saveVars = {'img', 'ksp', 'funVersion', 'acqp', 'method'...
            'faFids', 'faNoise', 'faKraw', 'faSats', 'idealHz'};
        else
            saveVars = {'img', 'ksp', 'funVersion', 'acqp', 'method'};
        end
