...
%RFCALBRUKER attempts to calibrate the excitation power for a Bruker fidall
%   or powcalSINGLEPULSE experiment acquired with variable RF power and
%   either EPI or spectral readouts (for fidall use an arbitrarily large
%   FOV [>999 cm] to null phase and readout gradients in normal readout mode)
%
%   Usage: [p90, bp90, mdl, gf] = rfCalBrukerFidall(inDir, [saveFiles, noi])
%
%   Inputs:
%     inDir     - Integer or char of fidall scan directory
%     saveFiles - If true save figures and workspace to scan directory
%                   (default = false)
%     noi       - Complex acquisition noise
%                   (only used if not acquired in fidall scan)
%
%   Outputs:
...
if ~isempty(regexp(method.Method, 'fidall', 'once'))
    if strcmpi(method.KAM_VFAOn, 'no')
        error('RF calibration fidall scan must be run with variable flip angles')
    end
    if strcmpi(method.KAM_IdealFidEcho, 'yes')
        error('RF calibration fidall scan cannot be run with a fid echo')
    elseif strcmpi(method.KAM_IdealSatEcho, 'yes')
        error('RF calibration fidall scan cannot be run with a saturation echo')
    end
...
elseif ~isempty(regexp(method.Method, 'powcalSINGLEPULSE', 'once'))
    ...
    error('Not a fidall or powcalsinglepulse experiment')
end
...
if nSlice > 1
    warning(sprintf('%s:multiSlice', mfilename), ...
        'Using center slice of multislice fidall scan')
end
if ~isempty(noi)
