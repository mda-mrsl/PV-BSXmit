...
* A shell script to call the complied matlab executable function will be necessary. *bsTransmitGainShell* is the name of the shell script. Save the shell script to **${PVHOME}/prog/curdir/${USER}/ParaVision/macros**. If you changed the name of the executable function or the path where files are saved, you will have to modify the shell script for it to operate properly. 
